...
export function MakeSelector({
  ...
  const { data: makesResponse, isLoading } = useVehicleMakes(
    debouncedSearchQuery.trim() || undefined
  );

  const options: Option[] = useMemo(() => {
    if (!makesResponse?.data) return [];
    return makesResponse.data.map((make) => ({
      label: make.name,
      value: make.id,
    }));
  }, [makesResponse]);
  ...
  // Store the label for display even when options haven't loaded yet
  ...
  const displayLabel = selectedMake?.label || selectedLabel;
  ...
  return (
    <>
      <Pressable onPress={onOpen} disabled={disabled}>
        <Container style={[styles.selector, { borderColor }]}>
          <Regular
            flex={1}
            fontSize={16}
                    ...
                    debouncedSearchQuery.trim() ? (
                      <Container flex={1} pt={20} alignItems="center">
                        <Regular fontSize={16} color="Neutrals/400">
                          No makes found for &ldquo;{searchQuery}&rdquo;
                        </Regular>
                      </Container>
                    ) : null
                  }
