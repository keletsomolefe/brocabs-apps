...
interface CreateDriverScreenProps {
  filesApi: FilesApi;
  authApi: AuthApi;
...
export function CreateDriverScreen({
  ...
  authApi,
  onSuccess,
}: CreateDriverScreenProps) {
  const {
    control,
    handleSubmit,
    formState: { errors, isSubmitted },
  } = useForm<FormInput, any, FormFields>({
  ...
  const createProfileMutation = useMutation({
    mutationKey: ["create-driver-profile"],
    mutationFn: async (data: FormFields) => {
      ...
      try {
        return await authApi.authControllerCreateDriverProfile({
          createProfileDto: {
